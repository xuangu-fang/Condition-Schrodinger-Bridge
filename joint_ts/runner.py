...
def compute_div_gz(opt, dyn, ts, xs, policy, return_zs=False):
    ...
    g_ts = dyn.g(ts) # (batch size x time steps)
    g_ts = g_ts[:, None,None] # (batch-size x time-steps,1,1)
    gzs = g_ts * zs
    e = sample_e(opt, xs)
...
def compute_sb_nll_joint_train(opt, batch_x, dyn, ts, xs_f, zs_f, x_term_f, policy_b):
    ...
    if opt.condition:
        ...
        mask_target_repeat = final_mask.repeat(opt.interval,1,1)
    ...
    def sb_joint_train(self, opt):
        ...
        for it in range(opt.num_itr):
            ...
            for inner_it in range(INNER_ITER):
                ...
                update_mask_flag = True#(it % opt.mask_update_itr ==0)
            ...
            if opt.condition:
                ...
                '''
                policy_b.net.set_x_condi(self.dyn.x_condi.repeat(opt.interval,1,1))
                policy_b.net.set_side_info(self.dyn.observed_tp.repeat(opt.interval,1)\
                    , self.dyn.mask_condi.repeat(opt.interval,1,1))
